    ...
    protected Optional<TurnOver> findTurnOver(ReportPlayerAction action) {
        ...
        for (IReport report : getReports()) {
            if (report instanceof ReportReRoll) {
                ...
                if (state.getReportSkillRoll() != null && ReportId.RIGHT_STUFF_ROLL == state.getReportSkillRoll().getId()) {
                    ...
                    } else if (injury.getInjuryType() instanceof TTMHitPlayer && !injury.getDefenderId().equals(getActivePlayer())) {
                        return Optional.of(new TurnOver(turnOverDescription.get(ReportId.THROW_TEAM_MATE_ROLL), 0, state.getReportReRoll(), getActivePlayer()));
                    }
                    break;
                }

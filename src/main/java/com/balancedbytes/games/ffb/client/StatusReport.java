...
public class StatusReport {
    ...
    public void reportVersion() {
        ...
        status.append("FantasyFootballClient Version ").append("1.2.5");
    ...
    public void reportScatterBall(ReportScatterBall pReport) {
        ...
        if (ArrayTool.isProvided(rolls = pReport.getRolls())) {
            ...
            for (int i2 = 0; i2 < directions.length; ++i2) {
                ...
                status.append(directions[i2].getName());
            }
            this.println(this.getIndent(), TextStyle.ROLL, status.toString());
        }
        if (pReport.isGustOfWind()) {
    ...
    public void reportKickoffPitchInvasion(ReportKickoffPitchInvasion pReport) {
        ...
        for (int i2 = 0; i2 < awayPlayers.length; ++i2) {
            if (rollsAway[i2] <= 0) continue;
            StringBuilder status = new StringBuilder();
            status.append("Pitch Invasion Roll [ ").append(rollsAway[i2]).append(" ]");
            this.println(this.getIndent(), TextStyle.ROLL, status.toString());
            this.print(this.getIndent() + 1, false, awayPlayers[i2]);
            status = new StringBuilder();
            ...
            int total = rollsAway[i2] + gameResult.getTeamResultHome().getFame() + fanFavouritesHome;
            status.append(" (Roll ").append(rollsAway[i2]);
    ...
    public void reportMostValuablePlayers(ReportMostValuablePlayers pReport) {
        ...
        for (String playerId2 : pReport.getPlayerIdsHome()) {
            player = game.getPlayerById(playerId2);
        ...
        for (String playerId2 : pReport.getPlayerIdsAway()) {
            player = game.getPlayerById(playerId2);
            this.print(this.getIndent() + 1, TextStyle.NONE, "The jury voted ");
    ...
    public void report(ReportList pReportList) {
        block81 : for (IReport report : pReportList.getReports()) {
            switch (report.getId()) {
    ...
    private void print(ParagraphStyle pParagraphStyle, TextStyle pTextStyle, String pText) {
        this.getClient().getUserInterface().getLog().append(pParagraphStyle, pTextStyle, pText);
    }
    ...
    private void println(ParagraphStyle pParagraphStyle, TextStyle pTextStyle, String pText) {
        this.print(pParagraphStyle, pTextStyle, pText);
        this.getClient().getUserInterface().getLog().append(null, null, null);
    }

...
public final class UtilCards {
    ...
    public static Card[] findAllActiveCards(Game pGame) {
        ...
        for (Card card2 : pGame.getTurnDataHome().getInducementSet().getActiveCards()) {
            allActiveCards.add(card2);
        }
        for (Card card2 : pGame.getTurnDataAway().getInducementSet().getActiveCards()) {
            allActiveCards.add(card2);
        }

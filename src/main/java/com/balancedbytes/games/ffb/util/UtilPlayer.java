...
public class UtilPlayer {
    ...
    public static int findTimmmberAssists(Game pGame, Player timmmberPlayer) {
        int assists = 0;
        Team opposingTeam = UtilPlayer.findOtherTeam(pGame, timmmberPlayer);
        FieldCoordinate coordinatePlayer = pGame.getFieldModel().getPlayerCoordinate(timmmberPlayer);
        for (Player assist : UtilPlayer.findAdjacentPlayersWithTacklezones(pGame, timmmberPlayer.getTeam(), coordinatePlayer, false)) {
            FieldCoordinate assistCoordinate = pGame.getFieldModel().getPlayerCoordinate(assist);
            Player[] opponents = UtilPlayer.findAdjacentPlayersWithTacklezones(pGame, opposingTeam, assistCoordinate, false);

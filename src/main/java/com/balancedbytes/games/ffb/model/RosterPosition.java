...
public class RosterPosition implements IJsonSerializable {
    ...
    public JsonObject toJsonValue() {
        ...
        for (SkillCategory skillCategory2 : this.getSkillCategories(true)) {
            skillCategoriesDouble.add(UtilJson.toJsonValue(skillCategory2));
        }
    ...
    public RosterPosition initFrom(JsonValue pJsonValue) {
        ...
        for (int i2 = 0; i2 < skillCategoriesDouble.size(); ++i2) {
            this.fSkillCategoriesOnDoubleRoll.add((SkillCategory)UtilJson.toEnumWithName(skillCategoryFactory, skillCategoriesDouble.get(i2)));
        }
        ...
        if (skillArray != null && skillArray.size() > 0 && ArrayTool.isProvided(skillValues)) {
            ...
            for (int i3 = 0; i3 < skillArray.size(); ++i3) {
                Skill skill = (Skill)UtilJson.toEnumWithName(skillFactory, skillArray.get(i3));
                this.fSkillValues.put(skill, skillValues[i3]);
            }
        }
        return this;
    }
